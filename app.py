...
# ========================= FLASK APP =========================
app = Flask(__name__)
app.secret_key = "pneumonia_detection_secret_key"
...
def load_dl_model():
    ...
    if os.path.exists(model_path):
        ...
        print("⚠️ pneumonia_model.h5 not found.")
        model = None
...
def model_page():
    ...
    has_metrics = os.path.exists(
        "static/images/accuracy_history.png"
    )
...
def is_likely_chest_xray(image_path):
    ...
    try:
        ...
        # X-rays should be mostly grayscale
        if color_score > 12:
        ...
        # Detect green/red/blue UI elements
        # =====================================
        ...
        # Even small colorful UI regions rejected
        if color_ratio > 0.01:
        ...
        # Screenshots/documents have many edges
        ...
        # Screenshots contain many straight lines
        if line_count > 120:
...
def predict():
    ...
    try:
        ...
        if not is_likely_chest_xray(temp_path):
            ...
            flash(
                "❌ Invalid Medical Scan Detected. "
                "Please upload a valid Chest X-Ray image only.",
                "danger"
            )
    ...
    try:
        ...
        try:
            ...
            save_and_display_gradcam(
                temp_path,
                heatmap,
                cam_path=gradcam_path
            )

            gradcam_rel_path = (
                f"/static/uploads/{gradcam_filename}"
            )
        ...
        final_filename = secure_filename(
            f"scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}"
        )
        ...
        original_rel_path = (
            f"/static/uploads/{final_filename}"
        )

        # ================= SAVE TO DATABASE (WITH USER_ID) =================
        record_id = db.log_prediction(
            user_id=session['user_id'],           # <--- UPDATED: pass user_id
...
def result_page(record_id):
    ...
    explanations = {
        "Normal":
            "The scan exhibits clear lungs with no visual indications of infection.",

        "Bacterial Pneumonia":
            "The scan shows focal consolidations consistent with bacterial pneumonia.",

        "Viral Pneumonia":
            "The scan shows diffuse infiltrates matching viral pneumonia patterns."
    }
...
def download_report(record_id):
    ...
    try:
        ...
        title = Paragraph(
            """
            <font size=22 color='#0d9488'>
            <b>PNEUMONIA DIAGNOSTIC SCAN REPORT</b>
            </font>
            """,
            styles['Title']
        )
        ...
        summary_title = Paragraph(
            "<font size=16 color='#0d9488'><b>Diagnostic Summary</b></font>",
            styles['Heading2']
        )
        ...
        image_title = Paragraph(
            "<font size=16 color='#0d9488'><b>Radiograph & Grad-CAM Heatmap</b></font>",
            styles['Heading2']
        )
        ...
        if record.get("gradcam_path"):
            gradcam_img_path = record["gradcam_path"].lstrip('/')
        ...
        disclaimer_title = Paragraph(
            "<font size=14 color='red'><b>CLINICAL DISCLAIMER</b></font>",
            styles['Heading3']
        )
        ...
        disclaimer_text = Paragraph(
            """
            This AI-generated report is for educational and supportive
            clinical workflow purposes only. All findings must be reviewed
            and confirmed by a certified medical professional or radiologist.
        ...
        footer = Paragraph(
            ...
            © 2026 PneuVision AI • Deep Learning Powered Diagnostic Support
            </font>
            """,
            styles['BodyText']
        )
        ...
        return send_file(
            pdf_path,
            as_attachment=True,
            download_name=f"pneumonia_report_{record_id}.pdf"
        )
